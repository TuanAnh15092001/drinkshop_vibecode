...
```typescript
addNewList(vars: AddNewListVariables): MutationPromise<AddNewListData, AddNewListVariables>;
...
```typescript
addNewList(dc: DataConnect, vars: AddNewListVariables): MutationPromise<AddNewListData, AddNewListVariables>;
...
```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, addNewList, AddNewListVariables } from '@dataconnect/generated';

// The `AddNewList` mutation requires an argument of type `AddNewListVariables`:
...
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await addNewList(addNewListVars);
// Variables can be defined inline as well.
const { data } = await addNewList({ name: ..., description: ..., public: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await addNewList(dataConnect, addNewListVars);

console.log(data.list_insert);

// Or, you can use the `Promise` API.
addNewList(addNewListVars).then((response) => {
  const data = response.data;
  console.log(data.list_insert);
});
```
...
import { connectorConfig, addNewListRef, AddNewListVariables } from '@dataconnect/generated';
...
const ref = addNewListRef(addNewListVars);
// Variables can be defined inline as well.
const ref = addNewListRef({ name: ..., description: ..., public: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = addNewListRef(dataConnect, addNewListVars);
